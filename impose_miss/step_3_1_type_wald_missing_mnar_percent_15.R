...
dt.mnar <- miss.apply.do(dt.full.X, b.trt=log(1), b.y=log(10), b.X=log(10), do=0.15)

dt.mnar.check <- dt.mnar%>%
  dplyr::mutate(do.H0 = purrr::map2(t.H0.m, 0.15, check.miss),
                do.H0 = purrr::map_dbl(do.H0, as.numeric),
                do.H1 = purrr::map2(t.H1.m, 0.15, check.miss),
                do.H1 = purrr::map_dbl(do.H1, as.numeric))%>%
  dplyr::select(-t.H0.m, -t.H1.m)


saveRDS(dt.mnar.check,file = sprintf('walddochmnar15_%d.rds',idx))

saveRDS(dt.mnar,file = sprintf('dtwaldmnar15_%d.rds',idx))
...
saveRDS(check.mech.mnar,file = sprintf('mechmnarwald15_%d.rds',idx))

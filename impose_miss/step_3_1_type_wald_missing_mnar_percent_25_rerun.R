...
dt.mnar <- miss.apply.do(dt.full.X, b.trt=log(1), b.y=log(10), b.X=log(10), do=0.25)

dt.mnar.check <- dt.mnar%>%
  dplyr::mutate(do.H0 = purrr::map2(t.H0.m, 0.25, check.miss),
                do.H0 = purrr::map_dbl(do.H0, as.numeric),
                do.H1 = purrr::map2(t.H1.m, 0.25, check.miss),
                do.H1 = purrr::map_dbl(do.H1, as.numeric))%>%
  dplyr::select(-t.H0.m, -t.H1.m)


saveRDS(dt.mnar.check,file = sprintf('walddochmnar25_%d.rds',idx))

saveRDS(dt.mnar,file = sprintf('dtwaldmnar25_%d.rds',idx))
...
saveRDS(check.mech.mnar,file = sprintf('mechmnarwald25_%d.rds',idx))

...
dt.mcar <- miss.apply.do(dt.full.X, b.trt=log(1), b.y=log(1), b.X=log(1), do=0.15)
...
saveRDS(dt.mcar.check,file = sprintf('walddochmcar15_%d.rds',idx))

saveRDS(dt.mcar,file = sprintf('dtwaldmcar15_%d.rds',idx))
...
saveRDS(check.mech.mcar,file = sprintf('mechmcarwald15_%d.rds',idx))

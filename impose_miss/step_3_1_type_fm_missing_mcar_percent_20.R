...
dt.mcar <- miss.apply.do(dt.full.X, b.trt=log(1), b.y=log(1), b.X=log(1), do=0.20)
...
saveRDS(dt.mcar.check,file = sprintf('fmdochmcar20_%d.rds',idx))

saveRDS(dt.mcar,file = sprintf('dtfmmcar20_%d.rds',idx))
...
saveRDS(check.mech.mcar,file = sprintf('mechmcarfm20_%d.rds',idx))

...
dt.mcar <- miss.apply.do(dt.full.X, b.trt=log(1), b.y=log(1), b.X=log(1), do=0.25)
...
saveRDS(dt.mcar.check,file = sprintf('wndochmcar25_%d.rds',idx))

saveRDS(dt.mcar,file = sprintf('dtwnmcar25_%d.rds',idx))
...
saveRDS(check.mech.mcar,file = sprintf('mechmcarwn25_%d.rds',idx))

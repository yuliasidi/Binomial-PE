...
##################################################################
##################################################################
# Repeat the above for smaller SS
##################################################################
...
bineval.20.sm <- data.frame(expand.grid(n1 = seq(10,89,1), p1 = seq(0.65,0.95,0.1)))%>%
  mutate(n2 = n1,
         p2= p1 - 0.2)
...
bineval.025.el.wald.sm  <- 
  bind_cols(bineval.025.sm,el = furrr::future_pmap_dbl(bineval.025.sm,el_2prop.wald, alpha=0.025,
                                                      .progress = TRUE))

system.time({
  bineval.10.el.wald.sm  <- 
    bind_cols(bineval.10.sm,el = furrr::future_pmap_dbl(bineval.10.sm,el_2prop.wald, alpha=0.025,
                                                     .progress = TRUE))
})

bineval.20.el.wald.sm  <- 
  bind_cols(bineval.20.sm,el = furrr::future_pmap_dbl(bineval.20.sm,el_2prop.wald, alpha=0.025,
                                                       .progress = TRUE))


saveRDS(bineval.025.el.wald.sm, "True_CP/summary/Expected_length/bineval.025.el.wald.sm.rds")
saveRDS(bineval.10.el.wald.sm, "True_CP/summary/Expected_length/bineval.10.el.wald.sm.rds")
saveRDS(bineval.20.el.wald.sm, "True_CP/summary/Expected_length/bineval.20.el.wald.sm.rds")
...
bineval.025.fm.sm <- bineval.025.sm%>%
  dplyr::mutate(M2 = 0.025)%>%
  as_tibble()%>%
  mutate(results = furrr::future_pmap(list(n1,p1,n2,p2,M2),
                                      cp_el_2prop.fm, alpha=0.025,
...
bineval.025.fm.all.sm <- bineval.025.fm.sm%>%
  dplyr::mutate(cp = map_dbl(results, .f=function(x) x$cp),
                el = map_dbl(results, .f=function(x) x$el),
                na.sum = map_dbl(results, .f=function(x) x$na.sum))%>%
  dplyr::select(-results)

bineval.10.fm.sm <- bineval.10.sm%>%
  dplyr::mutate(M2 = 0.1)%>%
  as_tibble()%>%
  mutate(results = furrr::future_pmap(list(n1,p1,n2,p2,M2),
                                      cp_el_2prop.fm, alpha=0.025,
...
bineval.10.fm.all.sm <- bineval.10.fm.sm%>%
  dplyr::mutate(cp = map_dbl(results, .f=function(x) x$cp),
                el = map_dbl(results, .f=function(x) x$el),
                na.sum = map_dbl(results, .f=function(x) x$na.sum))%>%
  dplyr::select(-results)

bineval.20.fm.sm <- bineval.20.sm%>%
  dplyr::mutate(M2 = 0.2)%>%
  as_tibble()%>%
  mutate(results = furrr::future_pmap(list(n1,p1,n2,p2,M2),
                                      cp_el_2prop.fm, alpha=0.025,
...
bineval.20.fm.all.sm <- bineval.20.fm.sm%>%
  dplyr::mutate(cp = map_dbl(results, .f=function(x) x$cp),
                el = map_dbl(results, .f=function(x) x$el),
                na.sum = map_dbl(results, .f=function(x) x$na.sum))%>%
  dplyr::select(-results)
...
saveRDS(bineval.025.fm.all.sm, "True_CP/summary/bineval.025.fm.all.sm.rds")
saveRDS(bineval.10.fm.all.sm, "True_CP/summary/bineval.10.fm.all.sm.rds")
saveRDS(bineval.20.fm.all.sm, "True_CP/summary/bineval.20.fm.all.sm.rds")
...
bineval.025.el.wilson.sm <- bind_cols(bineval.025.sm,el = furrr::future_pmap_dbl(bineval.025.sm,
                                                                           el_2prop.wilson, alpha=0.025,
                                                                           .progress = T))

bineval.10.el.wilson.sm <- bind_cols(bineval.10.sm,el = furrr::future_pmap_dbl(bineval.10.sm,
                                                                         el_2prop.wilson, alpha=0.025,
                                                                         .progress = T))

bineval.20.el.wilson.sm <- bind_cols(bineval.10.sm,el = furrr::future_pmap_dbl(bineval.20.sm,
                                                                         el_2prop.wilson, alpha=0.025,
                                                                         .progress = T))



saveRDS(bineval.025.el.wilson.sm, "True_CP/summary/Expected_length/bineval.025.el.wilson.sm.rds")
saveRDS(bineval.10.el.wilson.sm, "True_CP/summary/Expected_length/bineval.10.el.wilson.sm.rds")
saveRDS(bineval.20.el.wilson.sm, "True_CP/summary/Expected_length/bineval.20.el.wilson.sm.rds")

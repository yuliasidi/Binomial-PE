...
bineval <- readRDS("bineval.025.rds")
...
cp_2prop.fm <- function(n1,p1,n2,p2,M2,alpha){
  ...
  # if(any(is.na(dt.bin$w_flag),dt.bin$w_flag))
  #   message(sprintf('rows are Problematic:\n%s',
...
saveRDS(bineval.idx.cp, file = sprintf('binevalcp_fm_%02d.rds',idx))

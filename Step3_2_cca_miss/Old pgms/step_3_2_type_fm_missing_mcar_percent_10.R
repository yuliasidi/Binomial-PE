...
dt.miss <- readRDS(file = sprintf('dtfmmcar10_%d.rds',idx))
...
dt.miss <- dt.miss%>%
  dplyr::mutate(t.H0.CI = pmap(list(df=t.H0.m, M2 =  M2), FM.CI),
                t.H1.CI = pmap(list(df=t.H1.m, M2 =  M2), FM.CI),
...
saveRDS(dt.miss.sum,file = sprintf('ccamcarfm10_%d.rds',idx))

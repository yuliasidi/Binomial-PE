...
dt.miss <- readRDS(file = sprintf('dtwnmnar25_%d.rds',idx))
...
dt.miss <- dt.miss%>%
  dplyr::mutate(t.H0.CI = pmap(list(df=t.H0.m, M2 =  M2), wn.CI),
                t.H1.CI = pmap(list(df=t.H1.m, M2 =  M2), wn.CI),
...
saveRDS(dt.miss.sum,file = sprintf('ccamnarwn25_%d.rds',idx))

...
ff.n1 <- ff.n%>%filter(scenario%in%c(3,5,21), do ==10)



t1.H0 <- pmap_df(as.list(ff.n1), .f = function(scenario, do){
  t <-readRDS(sprintf("cluster/out/fm/mi5/outH0_m30_fm%d_%d_mi5.rds", scenario, do))
  t1 <- t%>%
    dplyr::bind_rows()
...
#type1.plot.20.mar.ccamice <-
  t2.H0%>%
  filter(strategy%in%c("cca", "mice m=5"), missing!="mcar", scenario.id==)%>%
...
t2.H0%>%
  ...
  dplyr::filter(do==10&!is.na(missing.new))%>%
  ggplot(aes(y=missing.new,x=type1,colour=strategy)) + 
  geom_point() + 
  facet_wrap(~scenario.id) + 
  geom_vline(xintercept=c(0.9,1.1)*0.025,
             linetype=2) + 
  labs(x = "Empirical Type-I error",
       y = "Drop-out difference (%C-%T)" ) +

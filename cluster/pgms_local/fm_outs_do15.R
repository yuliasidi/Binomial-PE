  ...
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/fm/2xcont/do15", 
                                    sprintf("cont2xH0_fm_sing_sc%s_do15_param1.rds", sc), 
                                    full.names = T))
           full.check(df, sc)
         })%>%
...
h0.mice.15 <- map_df(ll, 
                   .f = function(sc) {
                     df <- readRDS(list.files("cluster/out/fm/2xcont/do15", 
                                              sprintf("cont2xH0_fm_mice_sc%s_do15_param1.rds", sc), 
                                              full.names = T))
                     h0.mice.sum(df)%>%missing.desc.adj(do.adj = 15)
                   })%>%

...
ff.n1 <- ff.n%>%filter(scenario%in%c(3,11,13,15,17,19,21), do==20)%>%
  bind_rows(ff.n%>%filter(scenario%in%c(13,15,17,19,21), do==10))



t1.H0 <- pmap_df(as.list(ff.n1), .f = function(scenario, do, setn){
  t <-readRDS(sprintf("cluster/out/wald/mi5/outH0_m30_wald%d_%d_set%d_mi5.rds", scenario, do, setn))
  t1 <- t%>%
    dplyr::bind_rows()%>%
...
t2.H0$missing.new <- factor(t2.H0$missing.new,levels=unique(t2.H0$missing.new)[c(4,3,9,2,1,5,10,6,7,8)])
...
type1.plot.20.mar.ccamice <-
  ...
  plot.type1(do.val = 0.2, 
             p.title = "Type-I error: MAR for overall drop-out rate of 20%, Corr(Y,X)=-0.3")

bias.plot.20.mar.ccamice <-
    t2.H0%>%
    filter(strategy%in%c("cca", "mice m=5"), missing!="mcar")%>%
    plot.bias(do.val = 0.2, 
              p.title = "Bias: MAR for overall drop-out rate of 20%, Corr(Y,X)=-0.3")
...
type1.plot.10.mar.ccamice <-
  ...
  plot.type1(do.val = 0.1, 
             p.title = "Type-I error: MAR for overall drop-out rate of 10%, Corr(Y,X)=-0.3")

bias.plot.10.mar.ccamice <-
  t2.H0%>%
  filter(strategy%in%c("cca", "mice m=5"), missing!="mcar")%>%
  plot.bias(do.val = 0.1, 
            p.title = "Bias: MAR for overall drop-out rate of 10%, Corr(Y,X)=-0.3")


ff.n2 <- ff.n%>%filter(scenario%in%c(17,19,21))

t1.H1 <- pmap_df(as.list(ff.n2), .f = function(scenario, do, setn){
  t <-readRDS(sprintf("cluster/out/wald/mi5/outH1_m30_wald%d_%d_set%d_mi5.rds", scenario, do, setn))
  t1 <- t%>%
    dplyr::bind_rows()%>%
    dplyr::mutate(setn = setn)
  return(t1)
...
power.plot.20.mar.ccamice <-
  ...
  plot.power(do.val = 0.2, 
             p.title = "Power: MAR for overall drop-out rate of 20%, Corr(Y,X)=-0.3")
...
power.plot.10.mar.ccamice <-
  ...
  plot.power(do.val = 0.1, 
             p.title = "Power: MAR for overall drop-out rate of 10%, Corr(Y,X)=-0.3")
...
pdf("cluster/out/wald/plots/mi5/type1_mar10_wald_ccamice_m30.pdf")
type1.plot.10.mar.ccamice
dev.off()


pdf("cluster/out/wald/plots/mi5/type1_mar20_wald_ccamice_m30.pdf")
type1.plot.20.mar.ccamice
dev.off()
...
pdf("cluster/out/wald/plots/mi5/bias_mar10_wald_ccamice_m30.pdf")
bias.plot.10.mar.ccamice
dev.off()


pdf("cluster/out/wald/plots/mi5/bias_mar20_wald_ccamice_m30.pdf")
bias.plot.20.mar.ccamice
dev.off()
...
pdf("cluster/out/wald/plots/mi5/power_mar10_wald_ccamice_m30.pdf")
power.plot.10.mar.ccamice
dev.off()

pdf("cluster/out/wald/plots/mi5/power_mar20_wald_ccamice_m30.pdf")
power.plot.20.mar.ccamice
dev.off()

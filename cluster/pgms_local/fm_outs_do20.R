...
#check p_C, p_T and type1 for full data
full.type1 <-
  map_df(ll, 
         .f = function(sc) {
  ...
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/fm/2xcont/", 
                                    sprintf("cont2xH1_fm_sing_sc%s_do20_param1.rds", sc), 
                                    full.names = T))
           full.check(df, sc)
         })%>%
  dplyr::mutate(method = "fm")
...
saveRDS(full.power%>%
          dplyr::arrange(scenario.id), "cluster/out/overall/full.power.fm.rds")
...
do.check.do20.h1 <-
  ...
  dplyr::mutate(method = "fm", do = 0.2, hyp = "H1")

saveRDS(do.check.do20.h1%>%
          dplyr::arrange(scenario.id), "cluster/out/overall/do.check.fm.20.h1.rds")
...
h0.sing.do20 <-
  ...
  dplyr::filter(scenario.id!=19, scenario.id!=21)%>%
...
saveRDS(power.sing.20, "cluster/out/overall/h1.sing.fm.20.rds")
...
power.mice.20 <-
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/fm/2xcont/", 
                                    sprintf("cont2xH1_fm_mice_sc%s_do20_param1.rds", sc), 
                                    full.names = T))
           h0.mice.sum(df)%>%
             dplyr::select(-mean.bias)%>%
             dplyr::rename(power=type1)

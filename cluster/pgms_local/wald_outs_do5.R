  ...
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/wald/2xcont/do5", 
                                    sprintf("cont2xH0_wald_sing_sc%s_do5_param1.rds", sc), 
                                    full.names = T))
           full.check(df, sc)
         })%>%
...
saveRDS(h0.sing, "cluster/out/overall/h0.sing.wald.5.rds")
...
saveRDS(h0.mice, "cluster/out/overall/h0.mice.wald.5.rds")

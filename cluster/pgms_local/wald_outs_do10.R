  ...
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/wald/2xcont/do10", 
                                    sprintf("cont2xH0_wald_sing_sc%s_do10_param1.rds", sc), 
                                    full.names = T))
           full.check(df, sc)
         })%>%
...
do.check.do10.h1 <-
  ...
  dplyr::mutate(method = "wald", do = 0.10, hyp = "H1")
  ...
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/wald/2xcont/do10", 
                                    sprintf("cont2xH0_wald_sing_sc%s_do10_param1.rds", sc), 
                                    full.names = T))
           h0.sing.sum(df)%>%missing.desc.adj(do.adj = 10)
         })%>%
  dplyr::mutate(method = "wald")
...
saveRDS(h0.sing, "cluster/out/overall/h0.sing.wald.10.rds")
...
ll <- c(seq(1,5,1), seq(7,30,1))

h0.mice <-
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/wald/2xcont/do10", 
                                    sprintf("cont2xH0_wald_mice_sc%s_do10", sc), 
                                    full.names = T))
           h0.mice.sum(df)
         })%>%
...
saveRDS(h0.mice, "cluster/out/overall/h0.mice.wald.10.rds")
...
saveRDS(power.sing.10, "cluster/out/overall/h1.sing.wald.10.rds")

...
full.power.do15 <-
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/wn/2xcont/do15", 
                                    sprintf("cont2xH1_wn_sing_sc%s_do15_param1.rds", sc), 
                                    full.names = T))
           full.check(df, sc)
         })%>%
  dplyr::mutate(method = "wn")
...
do.check.do15.h1 <-
  ...
  dplyr::mutate(method = "wn", do = 0.15, hyp = "H1")

saveRDS(do.check.do15.h1%>%
          dplyr::arrange(scenario.id), "cluster/out/overall/do.check.wn.15.rds")
...
saveRDS(power.sing.15, "cluster/out/overall/h1.sing.wn.15.rds")
...
power.mice.15 <-
  map_df(ll, 
         .f = function(sc) {
           df <- readRDS(list.files("cluster/out/wn/2xcont/do15", 
                                    sprintf("cont2xH1_wn_mice_sc%s_do15_param1.rds", sc), 
                                    full.names = T))
           h0.mice.sum.wn(df)%>%
             dplyr::select(-mean.bias)%>%
             dplyr::rename(power=type1)
...
saveRDS(power.mice.15, "cluster/out/overall/h1.mice.wn.15.rds")

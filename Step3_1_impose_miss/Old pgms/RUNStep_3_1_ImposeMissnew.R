...
app_temp <- function(TYPE,MISSING,PERCENT){
  ...
  build_template(
    file = sprintf('step_3_1_type_%s_missing_%s_percent_%snew.R',TYPE,MISSING,PERCENT),
    args = c('$(Process)'),
    tag = sprintf('%s_%s_%snew',TYPE,MISSING,PERCENT),
    jobs = 30,
    init_dir = 'jobs/run',
    template_file = sprintf('impose_miss/step_3_1_type_%s_missing_%s_percent_%snew.condor',TYPE,MISSING,PERCENT),
...
scenarios1 <- scenarios%>%
  dplyr::mutate(fnameR = 
                  sprintf('impose_miss/step_3_1_type_%s_missing_%s_percent_%snew.R',TYPE,MISSING,PERCENT),
                fnameC = 
                  sprintf('impose_miss/step_3_1_type_%s_missing_%s_percent_%snew.condor',TYPE,MISSING,PERCENT),
                fnameC.submit = 
                  sprintf('step_3_1_type_%s_missing_%s_percent_%snew.condor',TYPE,MISSING,PERCENT))
...
condor::create_dirs(session, file = 'impose_miss/step_3_1_type_wald_missing_mnar_percent_25new.condor')
...
purrr::walk(seq(2,10,1), c.submit)

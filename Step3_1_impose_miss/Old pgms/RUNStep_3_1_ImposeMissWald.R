...
app_temp <- function(TYPE,MISSING,PERCENT){
  ...
  build_template(
    ...
    jobs = 30,
    init_dir = 'jobs/run',
    template_file = sprintf('Step3_1_impose_miss/step_3_1_type_%s_missing_%s_percent_%s.condor',TYPE,MISSING,PERCENT),
...
scenarios1 <- scenarios%>%
  dplyr::mutate(fnameR = 
                  sprintf('Step3_1_impose_miss/step_3_1_type_%s_missing_%s_percent_%s.R',TYPE,MISSING,PERCENT),
                fnameC = 
                  sprintf('Step3_1_impose_miss/step_3_1_type_%s_missing_%s_percent_%s.condor',TYPE,MISSING,PERCENT),
                fnameC.submit = 
...
condor::create_dirs(session, file = 'Step3_1_impose_miss/step_3_1_type_wald_missing_mnar_percent_25.condor')
...
purrr::walk(seq(1,15,1), c.submit)

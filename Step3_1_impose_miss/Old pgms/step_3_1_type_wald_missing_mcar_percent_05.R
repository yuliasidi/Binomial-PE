...
dt.mcar <- miss.apply.do(dt.full.X, b.trt=log(1), b.y=log(1), b.X=log(1), do=0.05)
...
saveRDS(dt.mcar.check,file = sprintf('walddochmcar05_%d.rds',idx))

saveRDS(dt.mcar,file = sprintf('dtwaldmcar05_%d.rds',idx))
...
saveRDS(check.mech.mcar,file = sprintf('mechmcarwald05_%d.rds',idx))

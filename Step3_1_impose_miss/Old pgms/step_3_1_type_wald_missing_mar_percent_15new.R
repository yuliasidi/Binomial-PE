...
dt.mar <- miss.apply.do(dt.full.X, b.trt=log(1), b.y=log(1), b.X=-log(10), do=0.15)
...
saveRDS(dt.mar.check,file = sprintf('walddochmar15_%d.rds',idx))

saveRDS(dt.mar,file = sprintf('dtwaldmar15_%d.rds',idx))
...
saveRDS(check.mech.mar,file = sprintf('mechmarwald15_%d.rds',idx))

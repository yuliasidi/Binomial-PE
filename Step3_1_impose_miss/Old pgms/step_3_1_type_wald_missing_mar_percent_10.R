...
dt.mar <- miss.apply.do(dt.full.X, b.trt=log(1), b.y=log(1), b.X=log(10), do=0.10)
...
saveRDS(dt.mar.check,file = sprintf('walddochmar10_%d.rds',idx))

saveRDS(dt.mar,file = sprintf('dtwaldmar10_%d.rds',idx))
...
saveRDS(check.mech.mar,file = sprintf('mechmarwald10_%d.rds',idx))

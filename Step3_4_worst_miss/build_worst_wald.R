...
purrr::pwalk(.l = scenarios_list,
             ...
             cat(
                  whisker::whisker.render(
                    readLines('Step3_4_worst_miss/anal_worst_wald.tmpl'),
                    data = list(TYPE = TYPE,
                                MISSING = MISSING,
                                PERCENT = PERCENT,
                                i = i)
                    ),
                  file = file.path('Step3_4_worst_miss/wald',
                                   sprintf('step_3_4_type_%s_missing_%s_percent_%s.R',TYPE,MISSING,PERCENT)
                                   ),
                  sep='\n')
             )

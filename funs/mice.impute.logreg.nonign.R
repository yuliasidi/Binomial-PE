mice.impute.logreg.nonign <- function (y, ry, x, wy = NULL, ...) 
{
  if (is.null(wy)) 
    wy <- !ry
  ...
  #default mice gives us ignorable phat, which is then "adjusted" by a random k
  p.ign <- 1/(1 + exp(-(x[wy, , drop = FALSE] %*% beta.star)))
  p <- k.mult*p.ign
  vec <- (runif(nrow(p)) <= p)
  vec[vec] <- 1
  if (is.factor(y)) {
